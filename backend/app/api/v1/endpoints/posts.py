...
from app.models import Fund, Giveaway, Post, Wallet
...
async def _enrich_post(post: Post, db: AsyncSession) -> PostResponse:
    """Add fund_label / giveaway_title and wallet_name to a post response."""
    fund_label = None
    giveaway_title = None
    wallet_name = None

    if post.fund_id is not None:
        result = await db.execute(select(Fund.label).where(Fund.id == post.fund_id))
        row = result.scalar_one_or_none()
        if row:
            fund_label = row
    if post.giveaway_id is not None:
        result = await db.execute(
            select(Giveaway.title).where(Giveaway.id == post.giveaway_id)
        )
        row = result.scalar_one_or_none()
        if row:
            giveaway_title = row
...
async def list_posts(
    ...
    giveaway_id: uuid.UUID | None = Query(
        None, description="Filter posts by giveaway ID"
    ),
    ...
    if fund_id is not None:
        ...
        query = query.where(Post.giveaway_id == giveaway_id)
...
async def create_post(
    ...
    giveaway_id: uuid.UUID | None = Query(
        None, description="Giveaway ID (alternative to fund_id)"
    ),
    db: AsyncSession = Depends(get_db),
    api_key: str = Depends(verify_api_key),
) -> PostResponse:
    """Create a new post linked to a fund or a giveaway."""
    effective_fund_id = body.fund_id or fund_id
    effective_giveaway_id = body.giveaway_id or giveaway_id

    if not effective_fund_id and not effective_giveaway_id:
        raise HTTPException(
            status_code=422,
            detail="Either fund_id or giveaway_id is required "
            "(provide in request body or as a query parameter)",
        )
    if effective_fund_id and effective_giveaway_id:
        raise HTTPException(
            status_code=422,
            detail="Provide only one of fund_id or giveaway_id, not both.",
        )

    if effective_fund_id:
    ...
    result = await db.execute(
        select(Giveaway).where(Giveaway.id == effective_giveaway_id)
    )
    giveaway = result.scalar_one_or_none()
    if not giveaway:
        raise HTTPException(status_code=404, detail="Giveaway not found")
    post = Post(body=body.body, giveaway_id=giveaway.id, wallet_id=giveaway.wallet_id)
    db.add(post)
    await db.commit()
    await db.refresh(post)
    logger.info("post_created", post_id=str(post.id), giveaway_id=str(giveaway.id))
    return await _enrich_post(post, db)
...
async def update_post(
    ...
    if body.fund_id is not None:
        ...
        post.giveaway_id = None
        post.wallet_id = new_fund.wallet_id
    elif body.giveaway_id is not None:
        g_result = await db.execute(
            select(Giveaway).where(Giveaway.id == body.giveaway_id)
        )
        new_giveaway = g_result.scalar_one_or_none()
        if not new_giveaway:
            raise HTTPException(status_code=404, detail="Giveaway not found")
        post.giveaway_id = new_giveaway.id
        post.fund_id = None
        post.wallet_id = new_giveaway.wallet_id
